...
import ru.tinkoff.gatling.amqp.action.{AmqpLogging, Around}
import ru.tinkoff.gatling.amqp.protocol.AmqpComponents
import ru.tinkoff.gatling.amqp.request.{AmqpDirectExchange, AmqpExchange, AmqpProtocolMessage, AmqpQueueExchange, AmqpTopicExchange}

import scala.collection.JavaConverters._

class AmqpPublisher(destination: AmqpExchange, components: AmqpComponents) extends WithAmqpChannel with AmqpLogging {
  def publish(message: AmqpProtocolMessage, around: Around, session: Session): Unit = {
    ...
    destination match {
      ...
      case AmqpTopicExchange(name, routingKey, durable) =>
        logger.info("publish: " + message.amqpProperties.getHeaders.get("correlation_id"))
        for {
          exName <- name(session)
          exKey <- routingKey(session)
        } withChannel { channel =>
